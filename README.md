...
* "experiments_pyroCb" contains the scripts used for the analyses in Chapter 6 of the thesis (and a small part of Appendix B).
    ...
    * "pyroCb_stabilized_classification_appendix" computes similar quantities as the script in the previous folder, but for 9 instead of 5 environments. These results are presented in the appendix. The data generated by the scripts in this folder are saved in the folder "saved_data". We print text files from the R sessionInfo() command called after each experiment in this script into the folder "sessionInfo".
