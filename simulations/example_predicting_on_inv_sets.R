...
# store the scores
nBCEs.rf <- ACCs.rf <- nBCEs.glm <- ACCs.glm <- matrix(0, nrow = B, ncol = 3)
names(nBCEs.rf) <- names(ACCs.rf) <- names(nBCEs.glm) <- names(ACCs.glm) <- c("{1,2}", "{1}", "{2}")
...
for(b in 1:B){
  ...
  nBCEs.rf[b, 1] <- nBCE(y = Y[(1+n.train):n], y.hat = preds.rf.12)
  ...
  nBCEs.rf[b, 2] <- nBCE(y = Y[(1+n.train):n], y.hat = preds.rf.1)
  ...
  nBCEs.rf[b, 3] <- nBCE(y = Y[(1+n.train):n], y.hat = preds.rf.2)
  ...
  nBCEs.glm[b, 1] <- nBCE(y = Y[(1+n.train):n], y.hat = preds.glm.12)
  ...
  nBCEs.glm[b, 2] <- nBCE(y = Y[(1+n.train):n], y.hat = preds.glm.1)
  ...
  nBCEs.glm[b, 3] <- nBCE(y = Y[(1+n.train):n], y.hat = preds.glm.2)
