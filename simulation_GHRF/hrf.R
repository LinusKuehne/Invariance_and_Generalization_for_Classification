...
ghrf.1 <- function(x, y, num.iter = NULL, kappa = 2) {
  ...
  trees.idx.df <- data.frame("first.half" = which(x$Env %in% envs.first.half),
                            "second.half" = which(!(x$Env %in% envs.first.half)))
  
  weights.idx.df <- data.frame("second.half" = which(!(x$Env %in% envs.first.half)),
                              "first.half" = which(x$Env %in% envs.first.half))
  
  rf.fits <- list()
  rf.weights <- list()
  
  for(i in 1:2){
    trees.idx <- trees.idx.df[,i]
    weights.idx <- weights.idx.df[,i]
    
    rf.fit.i <- ranger::ranger(x = x[trees.idx, 1:d], y = y[trees.idx], probability = TRUE, num.threads = 0)
    ...
    # Calculate residuals
    y.numeric <- as.numeric(y[weights.idx])-1
    rf.residuals <- y.numeric - rf.predictions.all
...
ghrf.2 <- function(x, y, num.iter = NULL, kappa = 2) {
  ...
  for(e in 1:num.env){
    ...
    rf.fit.e <- ranger::ranger(x = x[trees.idx, 1:d], y = y[trees.idx], probability = TRUE, num.threads = 0)
