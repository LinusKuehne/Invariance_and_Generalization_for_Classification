# in this script, we run stabilized classification on the pyroCb dataset with the residual test
...
for(e in 1:length(levels(envs))){
  ...
  # find the remaining environments
  ...
  for(s in 1:length(sets)){
    ...
    # run invariance test
    test.res <- residual(set = set, cube = X.train, labels = labels.train, y.num = y.num.train, group5 = train.env, group9 = train.env, cluster.assoc = event_df_train$clusters_env, posts)
    
    # store p-value for this subset
    pvals.e[s] <- test.res$p.val_fiveEnv
  }
...
for(e in 1:length(levels(envs))){
  ...
  # find indices for held-out environment
  i.test <- which(envs == levels(envs)[e])
  i.train <- -i.test
  ...
  for(s in 1:length(inv.sets)){
    ...
    # extract data for current invariant subset
...
for(e in 1:length(levels(envs))){
  ...
  # compute bootstrap prediction scores of best set
  boot.vec <- numeric(B)
  for(b in 1:B){
...
writeLines(capture.output(sessionInfo()), file.path(script_dir, "../sessionInfo/pyroCb_stabilized_classification_res5.txt"))
