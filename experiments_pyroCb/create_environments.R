...
# env7 (AZ NA) (actually env6 continued)
...
#loc_aus$Biome <- factor(loc_aus$Biome, levels = c(levels(loc_aus$Biome)[2], levels(loc_aus$Biome)[3], levels(loc_aus$Biome)[1]), ordered = T)
...
p_am <- ggmap(am_map) + 
  ...
  labs(title = "PyroCb in North America", color = "Environment and biome")+
...
p_aus <- ggmap(aus_map) + 
  ...
  xlab(expression(paste("Longitude (", degree,"E)"))) +
  ylab(expression(paste("Latitude (", degree,"N)"))) +
  labs(title = "PyroCb in Australia", color = "Environment and biome") +
  theme(legend.background = element_blank(), legend.key = element_blank()) +
  guides(color = guide_legend(ncol = 3))
...
g
ggsave(filename = "Map9.pdf", width = 8, height = 5.5)
...
#save(env5, env9, file = "discrete_envs.rdata")




cols <- c("black",  "hotpink", "blue1", "red", "darkorange")
...
p_am <- ggmap(am_map) + 
  ...
  xlab(expression(paste("Longitude (", degree,"E)"))) +
  ylab(expression(paste("Latitude (", degree,"N)"))) +
  labs(title = "PyroCb Events in North America", color = "Environment and biome")+
  theme(legend.background = element_blank(), legend.key = element_blank())+
  guides(color = guide_legend(ncol = 3))
...
p_aus <- ggmap(aus_map) + 
  ...
  xlab(expression(paste("Longitude (", degree,"E)"))) +
  ylab(expression(paste("Latitude (", degree,"N)"))) +
  labs(title = "PyroCb Events in Australia", color = "Environment and biome") +
  theme(legend.background = element_blank(), legend.key = element_blank())+
  guides(color = guide_legend(ncol = 3))
p_aus
...
ggsave(filename = "Map5.pdf", width = 8, height = 5.5)
