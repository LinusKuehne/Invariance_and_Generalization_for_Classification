...
weighted_accuracy <- function(labels, predictions) {
  
  pos.idx <- which(labels == 1)
  neg.idx <- which(labels == 0)
  
  pos.acc <- mean(labels[pos.idx] == predictions[pos.idx])
  neg.acc <- mean(labels[neg.idx] == predictions[neg.idx])
  
  return((pos.acc + neg.acc)/2)
}
