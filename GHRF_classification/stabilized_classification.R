...
c.pred <- function(Smax, sample, a.pred, B = 100){
  ...
  for(b in 1:B){
    ...
    if(sum(Smax)>0.0001){
      ...
      boot.model <- ranger(y = as.factor(boot.sample$Y), x = boot.sample[, Smax, drop = F], probability = T, num.threads = 0)
...
model.trainer <- function(sets.train, sample, usage){
  ...
  for(s in 1:length(sets.train)){
    ...
    if(sum(set) < 0.0001){
      ...
      # fit random forest
      rf <- ranger(y = as.factor(sample$Y), x = sample[, set, drop = F], probability = T, num.threads = 0, num.trees = 1000)
...
stabilizedClassification <- function(sample, test, a.inv = 0.05, a.pred = 0.05, B = 100, verbose = T){
...
predict.stabClass.hrf <- function(stabClassMod, newsample){
  ...
  for(m in 1:length(opt.models)){
    ...
    if(!is.numeric(opt.models[[m]])){
      ...
      current.hrf.fit <- hrf.orig(y = as.factor(train.sample$Y), x = train.sample[, opt.sets[[m]], drop = F], rf.fit = current.rf.fit)
